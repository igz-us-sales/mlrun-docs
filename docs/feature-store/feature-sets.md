...
The files can reside on S3, NFS, Azure blob storage or on Iguazio platform. MLRun also supports the Google Cloud BigQuery as a data source. 

When working with Azure, you must define the AZURE_STORAGE_CONNECTION_STRING environment variable. Use pip install mlrun[azure-blob-storage] to update the configuration.

For working with S3, you must define the "AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY" and "AWS_BUCKET" variables. Use pip install mlrun[s3] to update the configuration.
